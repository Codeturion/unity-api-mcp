...
Supports **Unity 6** (per minor stream: 6000.0 … 6000.5), **Unity 2023**, and **Unity 2022 LTS**, each with its own database. Works with Claude Code, Cursor, Windsurf, or any MCP-compatible AI tool. No Unity installation required. Check the [supported versions](https://github.com/Codeturion/unity-api-mcp/releases/tag/db-v1) — new Unity releases are detected and built automatically every week.
...
See the full list of supported versions and databases on the [**db-v1 release page**](https://github.com/Codeturion/unity-api-mcp/releases/tag/db-v1) — the table there is regenerated by CI on every build. New Unity patches are detected and built automatically every Monday.
